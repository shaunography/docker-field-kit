...
alias psyncthing='sudo podman run -d --restart unless-stopped --security-opt label=disable --network host --name syncthing  -e PUID=0 -e PGID=0 -e TZ=Europe/London -v $HOME:$HOME -v /media/$USER:/media/$USER -v $HOME/.syncthing/config:/config linuxserver/syncthing'
